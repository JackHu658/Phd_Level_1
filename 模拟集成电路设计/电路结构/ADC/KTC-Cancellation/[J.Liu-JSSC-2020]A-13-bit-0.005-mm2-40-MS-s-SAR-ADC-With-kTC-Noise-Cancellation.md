...
## 摘要（Abstract）
本文针对逐次逼近寄存器（Successive Approximation Register, SAR）ADC 前端采样保持电路的 **$kT/C$ 采样噪声瓶颈** 提出一种 **$kT/C$ 噪声抵消** 技术。该方法通过在比较器前置放大器（preamp）与锁存器之间引入电容 $C_2$ 以及相位控制 $\phi_2$，实现对 $C_1$ 上采样噪声的抵消，并将 $C_2$ 的噪声按前置放大器增益 $A$ 衰减，使得在保持高分辨率的同时显著缩小输入电容规模。芯片采用 40 nm CMOS 工艺，输入电容仅 240 fF，面积 0.005 mm$^2$，在 40 MS/s 下实现 69 dB 以上的 SNDR（覆盖整个奈奎斯特带宽）。**核心贡献是：在不牺牲 SNR 的前提下，显著减小输入电容，从而降低输入驱动和参考缓冲的系统级成本。**

## 引言（Introduction）
SAR ADC 以结构简单、数字化程度高和能效优势著称，[图1] 展示其典型结构（电容阵列 + 比较器 + SAR 逻辑）。然而，任何带前端 S/H 的 ADC 都面临采样 $kT/C$ 噪声的基础限制。要达到高 SNR，传统做法是增大输入电容 $C_1$，例如文中指出为了在 2.2 V$_{pp,diff}$ 下达到 80 dB 的 $kT/C$ 噪声受限 SNR，差分输入电容需超过 2.6 pF，且每提升 1 bit 分辨率需约 4 倍电容。由此导致输入驱动器与参考缓冲功耗/面积成为系统瓶颈。

已有方法如环内缓冲（loop-embedded buffer）和连续时间（CT）SAR 可降低输入端负载，但仍需要较大内部采样电容或面临直流输入与带宽限制。本文提出的 $kT/C$ 噪声抵消技术同时 **支持直流输入、扩大带宽、降低输入电容规模**，并避免输入驱动器超高速结算需求，是对上述缺陷的系统化解决方案。
