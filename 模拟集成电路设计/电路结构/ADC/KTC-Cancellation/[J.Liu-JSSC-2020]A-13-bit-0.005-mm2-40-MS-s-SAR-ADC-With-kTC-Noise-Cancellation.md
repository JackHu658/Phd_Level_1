...
![alt text](Pictures/[J.Liu-JSSC-2020]A-13-bit-0.005-mm2-40-MS-s-SAR-ADC-With-kTC-Noise-Cancellation/image-11.png)

采用全差分结构以抑制共模误差（开关电荷注入、锁存回冲等）。与单端结构相比，$\phi_1$ 使 preamp 输入连接至输出，而不是交流地，实现 **输入端直接偏置抵消**，降低输出摆幅。关键实现要点如下：

1. 主时钟 800 MHz，对应 40 MS/s 采样率；$C_2$ 采样相位约 0.6 ns。
2. 单端 $C_1$ 仅 120 fF，采用桥式电容阵列保证单位电容 0.85 fF；加入 3 个冗余电容并进行校准以应对比较错误，采用前台校准缓解电容失配。
...
![alt text](Pictures/[J.Liu-JSSC-2020]A-13-bit-0.005-mm2-40-MS-s-SAR-ADC-With-kTC-Noise-Cancellation/image-12.png)

频谱测量结果：1.01 MHz 输入时 SNDR=70.8 dB、SFDR=86.5 dB；19.1 MHz 输入时 SNDR=69 dB、SFDR=79.2 dB。  
...
1. **$kT/C$ 噪声抵消使得CDAC电容可显著缩小，从而减小芯片面积以及驱动的设计压力。**
2. **残余噪声主要受 $\Delta t/\tau$ 影响，实际设计需在 preamp 带宽与窗口时间之间权衡。**
3. **在 40 MS/s、1.1 V 下实现 69 dB 以上 SNDR 与 0.005 mm$^2$ 的芯片面积，验证了该技术的系统级优势。**
